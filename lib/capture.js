...
function Capture() {
        ...
        data.toString().split("\r\n").forEach( function( line ) {
          if( ! line.match( /^#/ ) ) {
            ...
            nline.split(",").forEach( function( n ) {
              if( n.length > 0 ) {
                if( parseInt( n, 10 ) >= 0 )
                  packetLength[ sprintf('%04s', packetCounter.toString( 16 ) ) ] = parseInt( n, 10 );
                packetCounter += 1;
              }
            } );
          }
  ...
  packetEv.on( 'packet', function( addr, port, buf ) {
    console.log( { addr: addr, port: port, buf: buf } );
    if( buf.length > 0 ) {
      ( function() {
        var ptype = sprintf( "%02s%02s", buf[1].toString(16), buf[0].toString(16) );
